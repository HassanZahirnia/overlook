<x-filament-widgets::widget id="overlook-resources-widget" @class(['hidden' => ! $data])>
    @if($data)
        <ul class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            @foreach($data as $resource)
                <li class="rounded-xl border border-gray-200 dark:border-gray-700 relative h-24 bg-gradient-to-tr from-gray-100 via-white to-white dark:from-gray-900 dark:via-gray-800 dark:to-gray-800" wire:key="{{ $resource['name'] }}">
